...
export function createDailyControlSummary({
  ...
  if (trainedToday && hydrationPct < 70) coachingMessage = isSl ? 'Trening je vpisan. Zdaj dvigni hidracijo proti cilju.' : 'Training is logged. Bring hydration closer to target next.';
  else if (!trainedToday && !restToday) coachingMessage = isSl ? 'Ce danes treniras, vpisi prvo vajo. Ce je pocitek, ga oznaci.' : 'If you train today, log the first exercise. If it is recovery, mark rest day.';
  else if (Math.abs(calorieDelta) <= 150 && hydrationPct >= 85) coachingMessage = isSl ? 'Dan je dobro poravnan: kalorije in voda sta blizu cilja.' : 'Today is lined up well: calories and water are close to target.';
